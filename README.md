...
- 📲 **Notification qui va droit au but** — la notification reçue s'ouvre directement sur la carte Duo en un tap, et propose surtout **deux gros boutons natifs** — `🚫 Pas aujourd'hui` et `🕒 Peut-être plus tard` — pour répondre en un geste, sans même déverrouiller l'app en entier, et sans risque de confondre les deux (ce sont de vrais boutons séparés, pas du texte à toucher précisément).
...
- `data.actions` : deux actions `duo_response_<entry_id>_pas_aujourdhui` et `duo_response_<entry_id>_plus_tard`, rendues comme deux boutons natifs distincts par l'application mobile.
